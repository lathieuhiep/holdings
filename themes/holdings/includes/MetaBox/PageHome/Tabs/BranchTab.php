    ...
    public static function fields(): array
    {
        return [
            Field::make(
                'text',
                self::KEY_TITLE,
                esc_html__('Tiêu đề', 'holdings')
            )
                ->set_default_value(
                    esc_html__('Các Chi Nhánh', 'holdings')
                ),

            Field::make(
                'text',
                self::KEY_ARCHIVE_URL,
                esc_html__('Url', 'holdings')
            )
                ->set_help_text(
                    esc_html__('Nhập link điều hướng.', 'holdings')
                ),

            Field::make(
                'text',
                self::KEY_LIMIT,
                esc_html__('Số chi nhánh hiển thị', 'holdings')
            )
            ...
            Field::make(
                'select',
                self::KEY_ORDERBY,
                esc_html__('Sắp xếp theo', 'holdings')
            )
                ->set_options([
                    'menu_order' => esc_html__('Menu order', 'holdings'),
                    'ID' => esc_html__('Post ID', 'holdings'),
                    'date' => esc_html__('Publish date', 'holdings'),
                    'title' => esc_html__('Title', 'holdings'),
                ])
            ...
            Field::make(
                'select',
                self::KEY_ORDER,
                esc_html__('Thứ tự sắp xếp', 'holdings')
            )
                ->set_options([
                    'ASC' => esc_html__('Ascending', 'holdings'),
                    'DESC' => esc_html__('Descending', 'holdings'),
                ])
