 ...
 * @return void
 */
function holdings_load_text_domain(): void
{
    load_theme_textdomain('holdings', get_template_directory() . '/languages');
}
 ...
 * @return void
 */
function holdings_add_theme_support(): void
{
 ...
 * @return void
 */
function holdings_register_nav_menus(): void
{
    register_nav_menus([
        'main_menu_left' => esc_html__('Menu chính bên trái', 'holdings'),
        'main_menu_right' => esc_html__('Menu chính bên phải', 'holdings'),
        'main_menu_device' => esc_html__('Menu chính hiển thị cho thiết bị (mobile, tablet)', 'holdings'),
    ]);
}
 ...
 * @return void
 */
function holdings_output_fallback_favicon(): void
{
    if (!has_site_icon()) {
 ...
 * @return void
 */
function holdings_remove_unnecessary_assets(): void
{
 ...
 */
function holdings_disable_emojis_tinymce(array $plugins): array {
 ...
 */
function holdings_disable_gutenberg_editor(): bool {
    return false;
}
 ...
 * @return void
 */
function holdings_init(): void
{
    // Theme setup
    holdings_load_text_domain();
    holdings_add_theme_support();
    holdings_register_nav_menus();

    // Cleanup and optimization hooks
    add_action('wp_head', 'holdings_output_fallback_favicon');
    add_action('init', 'holdings_remove_unnecessary_assets');
    add_filter('tiny_mce_plugins', 'holdings_disable_emojis_tinymce');
    add_filter('xmlrpc_enabled', '__return_false');
    add_filter('use_block_editor_for_post_type', 'holdings_disable_gutenberg_editor');
    add_filter('use_widgets_block_editor', '__return_false');
}
add_action('after_setup_theme', 'holdings_init');